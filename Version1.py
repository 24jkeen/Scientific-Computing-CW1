...
def augmented( y, args, coll, pars):
        return append (coll(y[:-1], pars) , dot(transpose(args[1]), (subtract(y , args[0]))))  ## define system of eqns with the pseudo-arclength encoded



def zerocoll( f, N, x , pars ):
    D, t = cheb(N)
...
def run_cheb(f, N, U0, pars, vary_par, step_size, max_steps):
    ...
    x0 = fsolve(zerocoll, U0, pars)                                             ## solve for the first point
    ...
    #    return append (ODE(y[:-1], pars) , dot(transpose(args[1]), (subtract(y , args[0]))))
